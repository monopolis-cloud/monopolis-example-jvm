...
import verysecuresystems.ACCESS_TOKEN_PREFIX
...
class EnteringBuildingTest {
    ...
    fun `unknown user is not allowed into building`() {
        assertThat(
            env.enterBuilding("Rita", AccessToken(ACCESS_TOKEN_PREFIX + UUID.randomUUID())),
            hasStatus(NOT_FOUND)
        )
    }
    ...
    fun `rejects missing username in entry endpoint`() {
        assertThat(
            env.enterBuilding(null, AccessToken(ACCESS_TOKEN_PREFIX + UUID.randomUUID())),
